#ifndef GAME8_PYRAMID_H
#define GAME8_PYRAMID_H

#include "BoardGame_Classes.h"
#include <vector>

class Pyramid_AI_Player : public Player<char> {
public:
    Pyramid_AI_Player(string name, char symbol) : Player<char>(name, symbol, PlayerType::AI) {}

    Move<char>* get_move() {
        Board<char>* board = this->get_board_ptr();
        vector<vector<char>> b = board->get_board_matrix();


        vector<pair<int,int>> pyramidCells = {
            {2,2},
            {1,1}, {1,2}, {1,3},
            {3,0}, {3,1}, {3,2}, {3,3}, {3,4},
            {4,0}, {4,1}, {4,2}, {4,3}, {4,4}
        };

        for(auto cell : pyramidCells) {
            if(b[cell.first][cell.second] == ' ') {
                cout << this->get_name() << " (AI) plays at (" << cell.first << "," << cell.second << ")" << endl;
                return new Move<char>(cell.first, cell.second, this->get_symbol());
            }
        }

        return new Move<char>(0,0,this->get_symbol());
    }
};

class Pyramid_Board : public Board<char> {
private:
    bool pyramidGrid[5][5] = {
        {false, false, false, false, false},
        {false, false, true, false, false},
        {false, true, true, true, false},
        {true, true, true, true, true},
        {true, true, true, true, true}
    };
public:
    Pyramid_Board() : Board<char>(5, 5) {
        for (auto& row : board) for (auto& cell : row) cell = ' ';
    }

    bool update_board(Move<char>* move) override {
        int x = move->get_x(), y = move->get_y();
        if (x < 0 || x >= 5 || y < 0 || y >= 5 || !pyramidGrid[x][y] || board[x][y] != ' ') return false;
        board[x][y] = move->get_symbol();
        n_moves++;
        return true;
    }

    bool is_win(Player<char>* player) override {
        char symbol = player->get_symbol();
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 3; j++) {
                if (pyramidGrid[i][j] && pyramidGrid[i][j+1] && pyramidGrid[i][j+2] &&
                    board[i][j] == symbol && board[i][j+1] == symbol && board[i][j+2] == symbol) return true;
            }
        }
        for (int j = 0; j < 5; j++) {
            for (int i = 0; i < 3; i++) {
                if (pyramidGrid[i][j] && pyramidGrid[i+1][j] && pyramidGrid[i+2][j] &&
                    board[i][j] == symbol && board[i+1][j] == symbol && board[i+2][j] == symbol) return true;
            }
        }
        return false;
    }

    bool is_lose(Player<char>* player) override { return false; }

    bool is_draw(Player<char>* player) override {
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                if (pyramidGrid[i][j] && board[i][j] == ' ') return false;
            }
        }
        return true;
    }

    bool game_is_over(Player<char>* player) override {
        return is_win(player) || is_draw(player);
    }
};

class Pyramid_UI : public UI<char> {
public:
    Pyramid_UI() : UI<char>("Pyramid Tic-Tac-Toe", 3) {}

    Move<char>* get_move(Player<char>* player) override {
        if (player->get_type() == PlayerType::AI) {
            Pyramid_AI_Player* aiPlayer = dynamic_cast<Pyramid_AI_Player*>(player);
            if (aiPlayer) {
                return aiPlayer->get_move();
            }
        }

        int x, y;
        cout << player->get_name() << ", enter row and column (0-4): ";
        cin >> x >> y;
        return new Move<char>(x, y, player->get_symbol());
    }

    void display_board_matrix(const vector<vector<char>>& matrix) const override {
        if (matrix.empty()) return;
        int rows = matrix.size(), cols = matrix[0].size();
        cout << "\nPyramid Board:" << endl;
        cout << "    ";
        for (int j = 0; j < cols; ++j) cout << setw(cell_width + 1) << j;
        cout << "\n   " << string((cell_width + 2) * cols, '-') << "\n";

        bool pyramidGrid[5][5] = {
            {false, false, false, false, false},
            {false, false, true, false, false},
            {false, true, true, true, false},
            {true, true, true, true, true},
            {true, true, true, true, true}
        };

        for (int i = 0; i < rows; ++i) {
            cout << setw(2) << i << " |";
            for (int j = 0; j < cols; ++j) {
                if (!pyramidGrid[i][j]) {
                    cout << setw(cell_width) << "X" << " |";
                } else if (matrix[i][j] == ' ') {
                    cout << setw(cell_width) << "." << " |";
                } else {
                    cout << setw(cell_width) << matrix[i][j] << " |";  //  محجوزة
                }
            }
            cout << "\n   " << string((cell_width + 2) * cols, '-') << "\n";
        }
        cout << "Note: X = blocked cell, . = empty cell" << endl;
    }

    Player<char>** setup_players() override {
        Player<char>** players = new Player<char>*[2];

        cout << "\n=== Pyramid Tic-Tac-Toe ===" << endl;
        cout << "Play only on pyramid cells (marked with .)" << endl;

        cout << "\nChoose Player 1 type:" << endl;
        cout << "1. Human" << endl;
        cout << "2. AI Player" << endl;
        cout << "Choice: ";
        int choice1;
        cin >> choice1;

        string name1;
        cout << "\nEnter Player 1 name: ";
        getline(cin >> ws, name1);

        if (choice1 == 2) {
            players[0] = new Pyramid_AI_Player(name1, 'X');
        } else {
            players[0] = new Player<char>(name1, 'X', PlayerType::HUMAN);
        }

        cout << "\nChoose Player 2 type:" << endl;
        cout << "1. Human" << endl;
        cout << "2. AI Player" << endl;
        cout << "Choice: ";
        int choice2;
        cin >> choice2;

        string name2;
        cout << "\nEnter Player 2 name: ";
        getline(cin >> ws, name2);

        if (choice2 == 2) {
            players[1] = new Pyramid_AI_Player(name2, 'O');
        } else {
            players[1] = new Player<char>(name2, 'O', PlayerType::HUMAN);
        }

        cout << "\n" << name1 << " will use 'X'" << endl;
        cout << name2 << " will use 'O'" << endl;
        cout << "\nGame starting(Only play on pyramid cells)" << endl;

        return players;
    }
};

void runPyramidGame() {
    system("cls");
    Pyramid_Board* board = new Pyramid_Board();
    Pyramid_UI* ui = new Pyramid_UI();
    Player<char>** players = ui->setup_players();
    GameManager<char> game(board, players, ui);
    game.run();
    delete board; delete ui; delete players[0]; delete players[1]; delete[] players;
}

#endif